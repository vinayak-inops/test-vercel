...
export default function EmployeeInfoPanel({ 
  ...
  // New state for employee panel backend data
  const [employeePanelData, setEmployeePanelData] = useState<any>(null);
  ...
  const {
    ...
    url: `map/contract_employee/search?employeeID=${employeeId}`,
    method: 'GET',
    onSuccess: (data: any) => {
  ...
  const getAttendanceRequestData = () => {
    let reqYear = year || new Date().getFullYear();
    let reqMonth = month || (new Date().getMonth() + 1);
  ...
  const {
    ...
    dependencies: [employeeID, year, month]
  });
  ...
  // Build the backend URL for the panel data
  const buildPanelUrl = () => {
    let reqYear = year || new Date().getFullYear();
    let reqMonth = month || (new Date().getMonth() + 1);
    let reqEmployeeId = employeeID || employeeId;
    // Ensure month is always two digits
    const monthStr = reqMonth.toString().padStart(2, '0');
    return `http://localhost:3000/muster/punch/individual-punch/calendar/information?employeeId=${reqEmployeeId}&month=${monthStr}&year=${reqYear}`;
  };

  // Fetch the panel data when params change
  useEffect(() => {
    const url = buildPanelUrl();
    if (!employeeID && !employeeId) return;
    setEmployeePanelLoading(true);
    setEmployeePanelError(false);
    fetch(url)
      .then(res => {
      ...
      .then(data => {
        setEmployeePanelData(data);
        setEmployeePanelLoading(false);
      })
      .catch(err => {
        setEmployeePanelError(true);
        setEmployeePanelLoading(false);
      });
  }, [employeeID, employeeId, year, month]);
