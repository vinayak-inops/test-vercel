...
import { UserCheck, Hash, Edit3, Copy, ExternalLink, X } from "lucide-react"
import type { Dispatch, SetStateAction } from "react"
import { useState } from "react"

interface DesignationDetailPopupProps {
  open: boolean
...
export default function DesignationDetailPopup({ open, setOpen, designationData }: DesignationDetailPopupProps) {
  const data = {
    designationName: "Manager",
    designationCode: "D001",
    divisionCode: "Div01",
    designationDescription: "Handles team and operations.",
    status: "Active",
    ...designationData,
  }
    ...
    >
      <div className="space-y-8">
        ...
        <div className="relative">
          <div className="absolute inset-0 bg-gradient-to-r from-violet-50/50 to-purple-50/50 rounded-2xl"></div>
          <div className="relative flex items-start gap-4 p-6 rounded-2xl border border-gray-100/50 backdrop-blur-sm">
            ...
            <div className="flex-1 min-w-0">
              <h1 className="text-md font-semibold text-gray-900 truncate mb-1">{data.designationName}</h1>
              <div className="flex items-center gap-3">
                <span className="inline-flex items-center px-3 py-1 text-xs font-semibold bg-gradient-to-r from-violet-50 to-purple-50 text-violet-700 rounded-full border border-violet-200/50 shadow-sm">
        ...
        {/* Designation Information with enhanced cards */}
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            <h2 className="text-sm font-bold text-gray-800 tracking-wide uppercase">Designation Information</h2>
