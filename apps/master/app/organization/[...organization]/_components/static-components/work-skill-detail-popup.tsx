...
import { Award, Hash, Edit3, Copy, ExternalLink, X } from "lucide-react"
import type { Dispatch, SetStateAction } from "react"
import { useState } from "react"

interface WorkSkillDetailPopupProps {
  open: boolean
  ...
  return (
    <div className="group flex items-center justify-between py-3 px-4 rounded-lg hover:bg-white/60 transition-all duration-200 border border-transparent hover:border-gray-100 hover:shadow-sm">
      <div className="flex items-center gap-3">
        <div className="flex items-center justify-center w-8 h-8 rounded-lg bg-gradient-to-br from-yellow-50 to-amber-50 border border-yellow-200/50 group-hover:from-white group-hover:to-amber-50 group-hover:border-amber-300/50 transition-all duration-200">
          {icon}
        </div>
...
export default function WorkSkillDetailPopup({ open, setOpen, workSkillData }: WorkSkillDetailPopupProps) {
  const data = {
    workSkillTitle: "Electrician",
    workSkillCode: "WSK001",
    workSkillDescription: "Description of Work Skill",
    status: "Active",
    ...workSkillData,
  }
    ...
    >
      <div className="space-y-8">
        ...
        <div className="relative">
          <div className="absolute inset-0 bg-gradient-to-r from-yellow-50/50 to-amber-50/50 rounded-2xl"></div>
          <div className="relative flex items-start gap-4 p-6 rounded-2xl border border-yellow-100/50 backdrop-blur-sm">
            <div className="relative">
              <div className="flex items-center justify-center w-14 h-14 rounded-2xl bg-gradient-to-br from-yellow-500 to-amber-600 shadow-lg shadow-yellow-500/25">
            ...
            <div className="flex-1 min-w-0">
              <h1 className="text-md font-semibold text-gray-900 truncate mb-1">{data.workSkillTitle}</h1>
              <div className="flex items-center gap-3">
                <span className="inline-flex items-center px-3 py-1 text-xs font-semibold bg-gradient-to-r from-yellow-50 to-amber-50 text-yellow-700 rounded-full border border-yellow-200/50 shadow-sm">
                  {data.workSkillCode}
                </span>
        ...
        {/* Work Skill Information with enhanced cards */}
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            <h2 className="text-sm font-bold text-gray-800 tracking-wide uppercase">Work Skill Information</h2>
          ...
          <div className="relative">
            ...
            <div className="relative border border-gray-200/50 rounded-2xl p-2 backdrop-blur-sm shadow-sm">
              <div className="space-y-1">
                <InfoRow
                  label="Work Skill Title"
                  value={data.workSkillTitle}
                  icon={<Award className="w-4 h-4 text-yellow-500" />}
                />
                <InfoRow
                  label="Work Skill Code"
                  value={data.workSkillCode}
