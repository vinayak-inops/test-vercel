...
import { Users, Hash, Edit3, Copy, ExternalLink, X } from "lucide-react"
import type { Dispatch, SetStateAction } from "react"
import { useState } from "react"

interface EmployeeCategoryDetailPopupProps {
  open: boolean
  ...
  return (
    <div className="group flex items-center justify-between py-3 px-4 rounded-lg hover:bg-white/60 transition-all duration-200 border border-transparent hover:border-gray-100 hover:shadow-sm">
      <div className="flex items-center gap-3">
        <div className="flex items-center justify-center w-8 h-8 rounded-lg bg-gradient-to-br from-indigo-50 to-purple-50 border border-indigo-200/50 group-hover:from-white group-hover:to-purple-50 group-hover:border-purple-300/50 transition-all duration-200">
          {icon}
        </div>
...
export default function EmployeeCategoryDetailPopup({ open, setOpen, employeeCategoryData }: EmployeeCategoryDetailPopupProps) {
  const data = {
    employeeCategoryName: "WKM",
    employeeCategoryCode: "WKM",
    employeeCategoryDescription: "Testing category",
    status: "Active",
    ...
    >
      <div className="space-y-8">
        ...
        <div className="relative">
          ...
          <div className="relative flex items-start gap-4 p-6 rounded-2xl border border-indigo-100/50 backdrop-blur-sm">
            <div className="relative">
            ...
            <div className="flex-1 min-w-0">
              <h1 className="text-md font-semibold text-gray-900 truncate mb-1">{data.employeeCategoryName}</h1>
              <div className="flex items-center gap-3">
                <span className="inline-flex items-center px-3 py-1 text-xs font-semibold bg-gradient-to-r from-indigo-50 to-purple-50 text-indigo-700 rounded-full border border-indigo-200/50 shadow-sm">
                  {data.employeeCategoryCode}
                </span>
        ...
        {/* Employee Category Information with enhanced cards */}
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            <h2 className="text-sm font-bold text-gray-800 tracking-wide uppercase">Employee Category Information</h2>
            ...
            <div className="relative border border-gray-200/50 rounded-2xl p-2 backdrop-blur-sm shadow-sm">
              <div className="space-y-1">
                <InfoRow
                  ...
                  value={data.employeeCategoryName}
                  icon={<Users className="w-4 h-4 text-indigo-500" />}
                />
                <InfoRow
                  label="Employee Category Code"
                  value={data.employeeCategoryCode}
                ...
                <div className="group flex flex-col items-start py-3 px-4 rounded-lg hover:bg-white/60 transition-all duration-200 border border-transparent hover:border-gray-100 hover:shadow-sm">
                  ...
                  <span className="text-sm text-gray-900 font-mono font-semibold tracking-wide break-words whitespace-pre-line ml-11">
                    {data.employeeCategoryDescription}
                  </span>
                </div>
                <InfoRow
                  label="Status"
