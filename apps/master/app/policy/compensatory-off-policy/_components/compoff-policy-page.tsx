...
export default function CompOffPolicyPage() {
    ...
    const {
        ...
        url: "map/compoff_policy/search?tenantCode=Midhani",
        onSuccess: (data) => {
    ...
    const functionalityList = {
        ...
        buttons: {
            ...
            addnew: {
                ...
                function: () => router.push("/policy/compensatory-off-policy?mode=add"),
            },
            cancel: {
                label: "Cancel",
                status: false,
            ...
            actionEdit: {
                ...
                function: (id: string) => router.push(`/policy/compensatory-off-policy?mode=edit&id=${id}`),
            },
        },
    }

    // If in form mode, show the PremiumBeautifiedForm
    if (isFormMode) {
        return (
            <div className="px-12 py-4">
                {/* <PremiumBeautifiedForm /> */}
            </div>
        );
    }
    ...
    return (
        <div>
            <EnhancedHeader
                ...
                description={"Compensatory Off Policy Management"}
                IconComponent={Calendar}
                recordCount={data && data.length > 0 ? data.length : 0}
                organizationType={"Compensatory Off Policy"}
                lastSync={2}
                uptime={99.9}
            />
            {
